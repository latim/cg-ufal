...
void loadTextureFile(char const *filename) {
	...
	glGenTextures(1, &textura[_index]);
	glBindTexture(GL_TEXTURE_2D, textura[_index]);
...
void drawTemple() {
	...
	glPushMatrix();
		glBegin(GL_TRIANGLE_FAN);
			...
			glVertex3d(centro_x, centro_y, 3.799980f);
	...
	glPushMatrix();
		...
		glTranslatef(4.429941f, 12.0f, 3.799980f); 
		glColor3f(COLOR_DOOR_X, COLOR_DOOR_Y, COLOR_DOOR_Z);
	...
	glPushMatrix();
		...
		glEnable(GL_TEXTURE_GEN_T);		
		glBindTexture(GL_TEXTURE_2D, textura[2]);
		glTranslatef(5.719941f, 12.0f, 3.799980f); 
		glColor3f(COLOR_DOOR_X, COLOR_DOOR_Y, COLOR_DOOR_Z);
		glScalef(1.7, 2.0, 0.5);
		glutSolidCube(0.5);
		glDisable(GL_TEXTURE_GEN_S); 
